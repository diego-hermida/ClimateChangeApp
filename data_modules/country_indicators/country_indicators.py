...
class __CountryIndicatorsDataCollector(DataCollector):
    ...
    def _collect_data(self):
        ...
        if not self.advisedly_no_data_collected:
            ...
            self.state['update_frequency'] = self.config['MIN_UPDATE_FREQUENCY'] if self.state['end_date'] <= \
                    (datetime.datetime.today().year - 1) else self.config['MAX_UPDATE_FREQUENCY']
        else:
