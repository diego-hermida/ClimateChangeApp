...
from utilities.util import date_plus_timedelta_gt_now, date_to_millis_since_epoch, deserialize_date, serialize_date
...
def instance(log_to_file=True, log_to_stdout=True) -> DataCollector:
    ...
    if not __singleton or __singleton and __singleton.finished_execution():
        __singleton = __HistoricalWeatherDataCollector(log_to_file=log_to_file, log_to_stdout=log_to_stdout)
    return __singleton
        ...
        if self.state['missing_data_check']:
            ...
            if not self.state['missing_data_ids']:
                ...
                if not self.state['current_date']:
                    self.state['current_date'] = self.__query_date()
