...
from utilities.util import date_to_millis_since_epoch, deserialize_date
...
def instance(log_to_file=True, log_to_stdout=True) -> DataCollector:
    ...
    if not __singleton or __singleton and __singleton.finished_execution():
        __singleton = __EnergySourcesDataCollector(log_to_file=log_to_file, log_to_stdout=log_to_stdout)
    return __singleton


class __EnergySourcesDataCollector(DataCollector):
