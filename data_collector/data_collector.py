...
class DataCollectorThread(Thread):
    ...
    def run(self):
        ...
        """
        Message(MessageType.register, content=self.__data_collector).send(self.__channel, self.__condition)
        self.__data_collector.run()
        Message(MessageType.finished, content=self.__data_collector).send(self.__channel, self.__condition)
...
class DataCollector(ABC):
    ...
    def __initialize_states(self):
        ...
        self.__STATE_SAVED = TransitionState(name='STATE_SAVED', code=STATE_SAVED, next_state=self.__FINISHED,
                actions=self.__finish_execution)
        ...
        self.__DATA_COLLECTED = TransitionState(name='DATA_COLLECTED', code=DATA_COLLECTED,
                next_state=self.__DATA_SAVED, actions=self._save_data)
        ...
        self.__STATE_RESTORED = TransitionState(name='STATE_RESTORED', code=STATE_RESTORED,
                next_state=self.__PENDING_WORK_CHECKED, actions=self._has_pending_work)
    ...
    def is_runnable(self) -> bool:
        ...
        """
        return self.__transition_state == self.__INITIALIZED
    ...
    def finished_execution(self) -> bool:
        """
            ...
            ABORTED state).
        """
        return self.__transition_state == self.__FINISHED or self.__transition_state == self.__ABORTED
    ...
    def successful_execution(self):
        ...
        """
        return self.__transition_state == self.__FINISHED and self.check_result
    ...
    def remove_files(self):
        ...
        try:
            ...
            pass
        try:
            remove_log_file(self.__file_path)
        except FileNotFoundError:
            pass
        ...
        if self.__transition_state != self.__ABORTED:
            ...
            while self.__transition_state > self.__CREATED and self.__transition_state != self.__FINISHED:
                try:
                    ...
                    self.__state_transitions.append(self.__transition_state)
                    ...
                    if self.__state_transitions and self.__state_transitions[-1] >= self.__STATE_RESTORED:
                        self.state['error'] = get_exception_info(actions_error)
                    self.__transition_state = self.__ABORTED
                    self.__state_transitions.append(self.__transition_state)
                    break
        self.__close_db_resources()
    ...
    def expose_transition_states(self, who) -> list:
        ...
        return self.__state_transitions if type(who) is Supervisor else None
    ...
    def execute_actions(self, state: int, who) -> bool:
        ...
        if not type(who) is Supervisor:
            return False
        if state == self.__INITIALIZED and self.__INITIALIZED.actions:
            self.__INITIALIZED.actions()
        elif state == self.__STATE_RESTORED and self.__STATE_RESTORED.actions:
            self.__STATE_RESTORED.actions()
        elif state == self.__PENDING_WORK_CHECKED and self.__PENDING_WORK_CHECKED.actions:
            self.__PENDING_WORK_CHECKED.actions()
        elif state == self.__DATA_COLLECTED and self.__DATA_COLLECTED.actions:
            self.__DATA_COLLECTED.actions()
        elif state == self.__DATA_SAVED and self.__DATA_SAVED.actions:
            self.__DATA_SAVED.actions()
        elif state == self.__EXECUTION_CHECKED and self.__EXECUTION_CHECKED.actions:
            self.__EXECUTION_CHECKED.actions()
        elif state == self.__STATE_SAVED and self.__STATE_SAVED.actions:
            self.__STATE_SAVED.actions()
        return True
