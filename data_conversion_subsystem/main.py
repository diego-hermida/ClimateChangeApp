...
def main(log_to_file=True, log_to_stdout=True, log_to_telegram=None):
    ...
    _logger = get_logger(__file__, 'MainLogger', to_stdout=log_to_stdout, to_file=log_to_file,
                         subsystem_id=DCS_CONFIG['SUBSYSTEM_INSTANCE_ID'],
    ...
    if builtins.EXECUTION_ID is not None:
        ...
        _logger.warning(
            'Starting Data Conversion Subsystem. Execution ID is unknown. This may difficult debug operations.')
    ...
    s = DataConverterSupervisor(channel, condition, log_to_file, log_to_stdout, log_to_telegram)
