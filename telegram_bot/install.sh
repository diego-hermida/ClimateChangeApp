...
if ([ "$SKIP_DEPLOY" != "true" ] && [ "$SKIP_DEPLOY" != "false" ]) ||
   ([ "$FORCE_BUILD" != "true" ] && [ "$FORCE_BUILD" != "false" ]); then
     exit_with_message 1 "> usage: install.sh [FORCE_BUILD=true] [SKIP_DEPLOY=false] [TELEGRAM_CONFIGURATOR_DEPLOY_ARGS=<args>]
...
if [ "$(docker image ls | grep telegram_bot | wc -c | tr -d ' ')" == 0 ] || [ "$FORCE_BUILD" == "true" ]; then
    ...
    if [ $? != 0 ]; then
        exit_with_message 1 "> The Telegram Configurator image could not be built." $?;
    fi
...
if [ $? != 0 ]; then
        exit_with_message 1 "\n[ERROR] The Telegram Configurator did not exit normally. You should rerun this installer." $?;
